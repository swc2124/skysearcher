...
def new_rc(rc_fh=None):
    ...
    # Name the directory for mpi temporary tables (mpi_table_dir).
    mpi_table_dir = os.path.join(table_dir, 'groupfinder', 'mpi')
    config.set('PATH', 'mpi_table_dir', mpi_table_dir)
    ...
    config.set('Search Extent', 'annulus_scale', '0.025')

    # Set the starting and ending point of the annulus (a0 & a1).
    config.set('Search Extent', 'a0', str(-pi))
    config.set('Search Extent', 'a1',str(pi))
    ...
    config.set('Search Extent', 'annulus_phi_step', '10')
    ...
    config.set('Accept Reject', 'min_boxes', '1')
    config.set('Accept Reject', 'max_boxes', '1000')
    
    # Set the minimum value for xbox (contrast density) to be considered
    # for the search (xbox_min).
    config.set('Accept Reject', 'xbox_min', '3.0')

    # Set minimum number of annuluii segments a feature must be
    # (min_seg_size).
    config.set('Accept Reject', 'min_seg_size', '1')
    config.set('Accept Reject', 'max_seg_size', '100')

    #
    #
    config.set('Accept Reject', 'xbox_min_mu', '2')

    #
    #
    config.set('Accept Reject', 'xbox_min_fixed', '1')
    
    #
    #
    config.set('Accept Reject', 'min_log_nstars', '1.75')
